...
# fixed in Perl's Graph, particularly around refvertexed
#
...
@vs = $t->seen;
for my $seq (@vs) {
	ok $seq->seq; # NOT OK in version .70
}

@vs = $g->articulation_points; 
ok $vs[0]->seq; # OK in version .70
ok scalar @vs, 2;

my @cc = $g->connected_components;
for my $ref (@cc) {
	for my $seq (@$ref) {
		ok $seq->seq; # OK in version .70
	}
}

my @bs = $g->bridges;
ok $bs[0][0]->seq; # NOT OK in version .70
